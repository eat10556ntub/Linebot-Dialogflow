...
// 由學號查詢學生成績
//------------------------------------------
var getScore = async function(stuno){
    //存放結果
    let result;  

    //讀取資料庫
    await query('select a.stuno, b.course, a.score from score a, course b where a.couno=b.couno and a.stuno = $1', [stuno])
        .then((data) => {
            if(data.rows.length > 0){
...
//匯出
module.exports = {getStu, getScore};
```
...
app.post('/dialogflow', express.json(), (req, res) => {
    ...
    function askScore(){
        ...
        //呼叫API取得學生成績資料
        return student.getScore(stuno).then(data => {  
            if (data == -1){
                agent.add('找不到資料');
            }else if(data == -9){   
                agent.add('執行錯誤');  
    ...
    intentMap.set('ask score', askScore);    
